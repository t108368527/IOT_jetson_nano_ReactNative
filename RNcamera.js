...
export default class Camera extends React.Component {
  ...
  /** MQTT **/
  // client = new Mqtt.Client('140.109.20.33');
  // componentDidMount() {
  ...
  SampleFunction1 = async function(data) {
    // console.log(RedisClient.redisConnect('140.109.20.33:8987', 'iotclassj6xul6'));
    RedisClient.redisConnect('140.109.20.33:8987', 'iotclassj6xul6');
    const image =  base64.encode(data.uri);
    console.log(image);
    RedisClient.publish('iotclass', image);
    ...
    if (this.camera) {
      ...
      if(data){
        try{
          ...
          const image =  await base64.encode(data.uri);
          console.log(image);
          RedisClient.publish('iotclass', image);
        } catch (e) {
          console.error(e);
    ...
    if (this.camera && !isRecording) {
      try {
        ...
        if (promise) {
          ...
          console.warn('影片路徑為', data.uri);
          // const [date, unixTime] = [moment().format('YYYY/MM/DD'), moment().unix()];
          const dir = `${Rnf.ExternalStorageDirectoryPath}/DCIM/100MEDIA`;
          console.warn('目標路徑', dir);
          // this.setState({
          //   video_uri = data.uri,
          // });
          // await this.SampleFunction1(data.uri);
          // await Rnf.mkdir(dir);
          // await Rnf.moveFile(data.uri,dir);
          // if(isRecording){
          //   console.log("send data");
          //   this.SampleFunction1(data.uri);
          // }
          // console.log(await RTCIceCandidatePairChangedEvent(dir));
        }
      } catch (e) {
        console.error(e);
      }
